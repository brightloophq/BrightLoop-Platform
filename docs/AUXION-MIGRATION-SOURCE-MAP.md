...
| R9 | **Placeholder content mistaken for real.** `PLACEHOLDER_*` datasets + empty legal shells. | Shipping sample testimonials/prices as real. | The "Preview — sample content" banner was removed at the owner's request. Reputation is now Supabase-backed, but the CATALOG still reports `source: "placeholder"` — package names, prices and estimates on the live site are placeholder and are no longer labelled as such. Retire this row by making the catalog real. |
