...
import { getRuntimeServices, getExecutionRepositories, getCollaborationRepositories, getAiFoundationRepositories, getAiProviderRegistry } from "./repositories";
...
export async function buildAppContext(): Promise<AppContext | null> {
  ...
  const [services, execution, collaboration, ai] = await Promise.all([getRuntimeServices(), getExecutionRepositories(), getCollaborationRepositories(), getAiFoundationRepositories()]);
  return { services, actor, ids: newId, clock: systemClock, execution, collaboration, ai, aiProviders: getAiProviderRegistry() };
}
