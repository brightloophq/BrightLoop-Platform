...
import { getRuntimeServices, getExecutionRepositories, getCollaborationRepositories } from "./repositories";
...
export async function buildAppContext(): Promise<AppContext | null> {
  ...
  const [services, execution, collaboration] = await Promise.all([getRuntimeServices(), getExecutionRepositories(), getCollaborationRepositories()]);
  return { services, actor, ids: newId, clock: systemClock, execution, collaboration };
}
