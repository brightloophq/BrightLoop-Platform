...
import { getRuntimeServices, getExecutionRepositories, getCollaborationRepositories, getAiFoundationRepositories, getAiProviderRegistry, getKnowledgeRepositories, getEmbeddingProviderRegistry, getVectorStore, getStrategistRepositories } from "./repositories";
...
export async function buildAppContext(): Promise<AppContext | null> {
  ...
  const [services, execution, collaboration, ai, knowledge, vectorStore, strategist] = await Promise.all([getRuntimeServices(), getExecutionRepositories(), getCollaborationRepositories(), getAiFoundationRepositories(), getKnowledgeRepositories(), getVectorStore(), getStrategistRepositories()]);
  return { services, actor, ids: newId, clock: systemClock, execution, collaboration, ai, aiProviders: getAiProviderRegistry(), knowledge, embeddingProviders: getEmbeddingProviderRegistry(), vectorStore, strategist };
}
