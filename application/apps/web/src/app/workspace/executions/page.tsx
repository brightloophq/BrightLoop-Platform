  ...
  return (
    <>
      ...
      {data.executions.length === 0
        ? <EmptyState title="No executions yet" body="When your active workflows run, their executions appear here with status, duration and failures." />
        : <div className={styles.list}>{data.executions.map((e) => (
            <Link key={e.id} href={`/workspace/executions/${e.id}`} className={styles.row}>
              <div className={styles.rowMain}>
                <div className={styles.rowTitle}> {e.externalExecutionId}</div>
