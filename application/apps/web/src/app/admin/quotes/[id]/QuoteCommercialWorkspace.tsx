...
import { commercialQuoteSummary, type QuotePricingType, type QuoteRecurrenceCadence } from "@brightloop/domain";
...
type Item = {
  ...
  unit_amount: number | null; amount: number | null; sort: number;
  pricing_type: QuotePricingType; recurrence_cadence: QuoteRecurrenceCadence | null;
  optional: boolean; source_work_item_id: string | null; source_evidence_refs: unknown;
};
type Quote = {
...
export function QuoteCommercialWorkspace({ quote, sourceProposal }: { quote: Quote; sourceProposal: { id: string; checksum: string; envelope: unknown } | null }) {
  ...
  const [items, setItems] = useState<Item[]>(quote.quote_items);
  ...
  const summary = useMemo(() => {
    try {
      return commercialQuoteSummary(items.map((item) => ({ quantity: item.quantity, unitAmount: item.unit_amount, pricingType: item.pricing_type, recurrenceCadence: item.recurrence_cadence, optional: item.optional })), discount);
    } catch { return null; }
  }, [items, discount]);

  const update = (index: number, patch: Partial<Item>) => setItems((current) => current.map((item, i) => i === index ? { ...item, ...patch } : item));
  const add = () => setItems((current) => [...current, { id: null, label: "", description: "", quantity: 1, unit_amount: null, amount: null, sort: current.length, pricing_type: "one_time", recurrence_cadence: null, optional: false, source_work_item_id: null, source_evidence_refs: [] }]);
  ...
  async function save() {
    ...
    try {
      ...
      const body = await response.json() as { error?: string; updatedAt?: string };
  ...
  return <div className={styles.layout}>
    <OperationalPanel>
      ...
      <div className={styles.meta}>
        ...
        <label>One-time discount (minor units)<input type="number" min="0" step="1" value={discount} onChange={(event) => setDiscount(Number(event.target.value))} disabled={!editable} /></label>
    ...
    <OperationalPanel>
      ...
      <div className={styles.items}>{items.map((item, index) => <div className={styles.item} key={item.id ?? `new-${index}`}>
        <div className={styles.itemHead}><strong>{item.source_work_item_id ? "Scanner-derived scope" : "Operator scope"}</strong>{editable ? <button type="button" onClick={() => setItems((current) => current.filter((_, i) => i !== index))}>Remove</button> : null}</div>
    ...
    <OperationalPanel>
      <div className={styles.totals}>
        <span>Committed one-time subtotal</span><strong>{money(summary?.subtotal ?? 0, currency)}</strong>
        <span>Discount</span><strong>−{money(summary?.discount ?? 0, currency)}</strong>
        <span>Committed one-time total</span><strong>{money(summary?.total ?? 0, currency)}</strong>
        <span>Committed recurring</span><strong>{money(summary?.recurringTotal ?? 0, currency)} {summary?.recurringCadence ?? ""}</strong>
        <span>Optional one-time</span><strong>{money(summary?.optionalOneTimeTotal ?? 0, currency)}</strong>
        <span>Optional recurring</span><strong>{money(summary?.optionalRecurringTotal ?? 0, currency)}</strong>
      </div>
