/* =============================================================================
 * Social connectors — shared config, provider binding, request client (F4.7).
 *
 * The generic engine. A `SocialProviderBinding` is the ONLY provider-specific surface
 * — Meta, LinkedIn, X (Twitter), and TikTok are data-driven bindings over this one
 * engine. `authorize` resolves a per-call `AuthContext` (base URL + Bearer headers +
 * any provider version header) from the resolved OAuth access token + the
 * installation's non-secret config; `callSocial` then attaches those headers, encodes
 * the JSON body, drives the transport, and normalizes the response via the binding's
 * classifier. A separate `callTokenEndpoint` performs the form-encoded OAuth token
 * exchange. No secret/token/body ever leaks into an error. Server-only.
 *
 * The OAuth endpoint descriptor carries a few knobs the four social providers need
 * that the finance family did not: `clientIdParam`/`clientSecretParam` (TikTok names
 * its client credential `client_key`) and `scopeSeparator` (Meta + TikTok comma-join
 * scopes where LinkedIn + X space-join them).
 * ========================================================================== */
...
import type { SocialErrorClass } from "./errors.js";
import type { SocialHttpTransport } from "./transport.js";
...
export interface SocialOAuthEndpoints {
  authorizeEndpoint: string;
  tokenEndpoint: string;
  /** Space-joined scope param name (default `scope`). */
  scopeParam?: string;
  /** Separator joining scopes (default " "; Meta + TikTok use ","). */
  scopeSeparator?: string;
  /**
   * How client credentials are presented at the token endpoint: `body` (default —
   * client id/secret as form params, used by Meta/LinkedIn/TikTok) or `basic` (HTTP
   * Basic `client_id:client_secret`, used by X/Twitter confidential clients).
   */
  tokenAuthStyle?: "body" | "basic";
  /** The form/URL param name carrying the client id (TikTok uses `client_key`). */
  clientIdParam?: string;
  /** The form param name carrying the client secret. */
  clientSecretParam?: string;
  /** The authorize `response_type` (default `code`). */
  responseType?: string;
...
export interface SocialCallSpec {
...
export type SocialCall = (spec: SocialCallSpec) => Promise<ConnectorResult<Record<string, unknown>>>;
export type SocialOp = (call: SocialCall, input: OpInput, conn: OpInput) => Promise<ConnectorResult<OperationOutput>>;
export type SocialPollFn = (call: SocialCall, conn: OpInput, cursor: string | null, limit: number, now: () => string) => Promise<ConnectorResult<PollResult>>;
...
/** The ONE provider-specific surface. Meta / LinkedIn / X / TikTok each export one. */
export interface SocialProviderBinding {
  connectorId: string;
  /** OAuth endpoints (all four social providers use authorization-code OAuth 2.0). */
  oauth: SocialOAuthEndpoints;
  /** Classify a response; return null for success. */
  classify: (status: number, body: Record<string, unknown>) => SocialErrorClass | null;
  /**
   * Resolve the auth context (base URL + Bearer headers + version headers) for a call
   * from the resolved access token + non-secret config. Returns `secret_unavailable`
   * when no token is present and `config_invalid` when a required config field is
   * missing.
   */
...
function withQuery(url: string, query?: SocialCallSpec["query"]): string {
  if (!query) return url;
  const parts: string[] = [];
...
export async function callSocial(cfg: SocialConfig, binding: SocialProviderBinding, auth: AuthContext, spec: SocialCallSpec): Promise<ConnectorResult<Record<string, unknown>>> {
  ...
  try {
    ...
    if (e instanceof SocialTransportError) return connectorErr(e.kind === "timeout" ? "timeout" : "network", "transport error", e.kind);
...
export async function callTokenEndpoint(cfg: SocialConfig, url: string, form: Record<string, string>, classify: SocialProviderBinding["classify"], extraHeaders?: Record<string, string>): Promise<ConnectorResult<Record<string, unknown>>> {
  try {
