/* =============================================================================
 * CRM connectors — ConnectorAdapter assembly (F4.5). SERVER-ONLY.
 *
 * ONE generic engine builds a framework `ConnectorAdapter` from a `CrmProviderBinding`
 * (HubSpot/Salesforce/Pipedrive). Each call resolves an AuthContext (base URL +
 * Bearer headers) from the resolved OAuth token + install config, then runs the work
 * through a pre-authorized `call` closure. It composes the OAuth flow (authorize URL,
 * code exchange, refresh), a cheap probe (validate/health), capability discovery,
 * `execute` dispatch over the normalized crm.* operation map, polling + canonical
 * event translation, and webhook verify/translate where the provider supports it.
 * Provider-neutral out; no provider type or secret leaks past this boundary.
 * Disabled-safe: with no client creds an adapter still registers but OAuth fails
 * clearly and no token call is attempted.
 * ========================================================================== */
...
import { callCrm, type CrmCall, type CrmConfig, type CrmProviderBinding, type ProviderCreds } from "./client.js";
import { healthForReason, reasonForCategory } from "./errors.js";
import { buildCrmAuthorizationUrl, exchangeCrmCode, refreshCrmToken } from "./oauth.js";
...
const BINDINGS: CrmProviderBinding[] = [HUBSPOT_BINDING, SALESFORCE_BINDING, PIPEDRIVE_BINDING];

/** Shared config + per-provider OAuth client credentials keyed by connector id. */
export interface CrmConnectorConfig extends CrmConfig {
  creds: Partial<Record<string, ProviderCreds>>;
}

function createCrmAdapter(cfg: CrmConnectorConfig, binding: CrmProviderBinding): ConnectorAdapter {
  const creds = cfg.creds[binding.connectorId] ?? null;
  const declaredOps = Object.keys(binding.ops);
  const callWith = (auth: { baseUrl: string; headers: Record<string, string> }): CrmCall => (spec) => callCrm(cfg, binding, auth, spec);
  ...
  if (creds !== null) {
    base.buildAuthorizationUrl = (input: BuildAuthorizationUrlInput) => buildCrmAuthorizationUrl(cfg, binding, creds, input);
    base.exchangeAuthorizationCode = (input: ExchangeCodeInput) => exchangeCrmCode(cfg, binding, creds, input);
    base.refreshAccessToken = (input: RefreshTokenInput) => refreshCrmToken(cfg, binding, creds, input);
  }

  return base;
}

/** Build the three CRM connector adapters keyed by connector id. */
export function createCrmConnectorAdapters(cfg: CrmConnectorConfig): ConnectorAdapterRegistry {
  const registry: ConnectorAdapterRegistry = {};
  for (const binding of BINDINGS) registry[binding.connectorId] = createCrmAdapter(cfg, binding);
  return registry;
}

/**
 * Load the app-level CRM OAuth config from the environment. Client id/secret for each
 * provider are provisioned out-of-band (HubSpot/Salesforce/Pipedrive connected apps);
 * access + refresh tokens are per-install secrets stored ONLY by reference. Client
 * secrets are NEVER persisted.
 */
export function loadCrmConfig(env: NodeJS.ProcessEnv, transport: CrmConfig["transport"], now: () => string): CrmConnectorConfig {
  return {
    transport,
    now,
    ...
    timeoutMs: 15_000,
    creds: {
      hubspot: { clientId: env["HUBSPOT_CLIENT_ID"] ?? "", clientSecret: env["HUBSPOT_CLIENT_SECRET"] ?? "" },
      salesforce: { clientId: env["SALESFORCE_CLIENT_ID"] ?? "", clientSecret: env["SALESFORCE_CLIENT_SECRET"] ?? "" },
      pipedrive: { clientId: env["PIPEDRIVE_CLIENT_ID"] ?? "", clientSecret: env["PIPEDRIVE_CLIENT_SECRET"] ?? "" },
    },
  };
}
