/* =============================================================================
 * Integration Platform — F4.5 CRM connectors (application tests).
 *
 * Verifies the three CRM connectors are installable OAuth2 production connectors with
 * a NORMALIZED capability vocabulary, drives read + write capability invocation
 * through the real use-cases end-to-end, exercises OAuth connect + transparent
 * refresh/rotation before invocation + revoked-token reconnect, the webhook ingestion
 * pipeline (verify → translate → idempotent persist → REPLAY duplicate), polling
 * replay-safety, and authorization (clients cannot invoke; cross-tenant is denied) +
 * secret non-leak. A deterministic stub CRM adapter stands in for the data-layer
 * adapter so the application package stays domain+schema only.
 * ========================================================================== */
...
/**
 * A deterministic stub CRM adapter: OAuth2 (authorize URL / exchange / refresh),
 * secret-gated execute that echoes, a webhook verified by exact signature==signingSecret
 * match translating the body id into ONE canonical crm.contact.created event, and a
 * cursor-based poll. Mirrors the framework contract the real data-layer adapter
 * fulfils, without a transport. `opts.denyScope` forces an authorization failure;
 * `opts.expiredExchange` returns an already-expired token so refresh must run.
 */
function stubCrmAdapter(connectorId: string, opts: { denyScope?: boolean; expiredExchange?: boolean } = {}): ConnectorAdapter {
  ...
  return {
    ...
    async exchangeAuthorizationCode(i) { if (i.code.length === 0) return connectorErr("validation", "no code"); return connectorOk({ accessToken: `at-${i.code}`, refreshToken: `rt-${i.code}`, scopes: ["crm"], expiresAt: opts.expiredExchange ? "2020-01-01T00:00:00.000Z" : null, tokenType: "bearer" }); },
    async refreshAccessToken(i) { return connectorOk({ accessToken: "at-refreshed", refreshToken: i.refreshToken, scopes: ["crm"], expiresAt: "2027-01-01T00:00:00.000Z", tokenType: "bearer" }); },
    async execute(i) {
    ...
    translateWebhook(i): ConnectorResult<CanonicalConnectorEvent[]> {
      ...
      return connectorOk([{ type: "crm.contact.created", externalId: id, occurredAt: T0, payload: { objectId: id }, provenance: `${connectorId}:webhook` }]);
    },
    async poll(i) {
      const start = i.cursor === null ? 0 : Number.parseInt(i.cursor, 10) || 0;
      const events: CanonicalConnectorEvent[] = Array.from({ length: Math.min(i.limit, 2) }, (_v, k) => ({ type: "crm.deal.updated", externalId: `deal-${start + k}`, occurredAt: T0, payload: {}, provenance: `${connectorId}:poll` }));
...
let adapters: ConnectorAdapterRegistry = { hubspot: stubCrmAdapter("hubspot"), salesforce: stubCrmAdapter("salesforce"), pipedrive: stubCrmAdapter("pipedrive") };
...
beforeEach(() => {
  ...
  adapters = { hubspot: stubCrmAdapter("hubspot"), salesforce: stubCrmAdapter("salesforce"), pipedrive: stubCrmAdapter("pipedrive") };
  ctx = makeCtx(OWNER);
});

/** Install + OAuth-connect a CRM connector (token stored, status connected). */
async function connectCrm(connectorId: string, config: Record<string, unknown> = {}) {
...
describe("marketplace — HubSpot / Salesforce / Pipedrive are installable OAuth2 CRM connectors", () => {
  it("registers all three with normalized crm capabilities", () => {
    ...
    for (const id of ["hubspot", "salesforce", "pipedrive"]) {
      ...
      expect(byId[id]!.category, id).toBe("crm");
      const keys = byId[id]!.capabilities.map((c) => c.key);
      for (const k of ["crm.contacts.list", "crm.deals.list", "crm.deals.stage.update", "crm.health"]) expect(keys, id).toContain(k);
    }
  });
});
...
describe("capability invocation (read + write)", () => {
  it("invokes a read capability and records an audit row", async () => {
    const inst = await connectCrm("hubspot");
    const res = await invokeConnectorCapability(ctx, { installationId: inst.id, capabilityKey: "crm.contacts.list", input: { limit: 10 } });
    expect(res.connectorId).toBe("hubspot");
    expect(res.capabilityKey).toBe("crm.contacts.list");
  ...
  it("invokes a write capability (create) for an authorized owner", async () => {
    const inst = await connectCrm("pipedrive", { companyDomain: "acme" });
    const res = await invokeConnectorCapability(ctx, { installationId: inst.id, capabilityKey: "crm.contacts.create", input: { name: "Ada" } });
    expect(res.capabilityKey).toBe("crm.contacts.create");
  });
  it("a client actor cannot invoke a CRM capability", async () => {
    const inst = await connectCrm("hubspot");
    const clientCtx = { ...makeCtx(CLIENT), integration: repos, connectorAdapters: adapters, connectorSecrets: secrets };
    await expect(invokeConnectorCapability(clientCtx, { installationId: inst.id, capabilityKey: "crm.contacts.list", input: {} })).rejects.toBeInstanceOf(ForbiddenError);
  });
  it("a missing provider scope surfaces as a permission error", async () => {
    adapters = { ...adapters, hubspot: stubCrmAdapter("hubspot", { denyScope: true }) };
    ctx = makeCtx(OWNER);
    const inst = await connectCrm("hubspot");
    await expect(invokeConnectorCapability(ctx, { installationId: inst.id, capabilityKey: "crm.contacts.create", input: { email: "a@x.io" } })).rejects.toBeInstanceOf(ValidationError);
  });
});
...
describe("OAuth token lifecycle — refresh, rotation, reconnect", () => {
  it("transparently refreshes + rotates an expired access token before invoking", async () => {
    adapters = { ...adapters, hubspot: stubCrmAdapter("hubspot", { expiredExchange: true }) };
    ctx = makeCtx(OWNER);
    const inst = await connectCrm("hubspot");
    const ref = await oauthTokenRef(inst.id);

    const res = await invokeConnectorCapability(ctx, { installationId: inst.id, capabilityKey: "crm.contacts.list", input: {} });
    expect(res.capabilityKey).toBe("crm.contacts.list");
  ...
  it("surfaces reconnect when the stored token is revoked out-of-band", async () => {
    ...
    await expect(invokeConnectorCapability(ctx, { installationId: inst.id, capabilityKey: "crm.contacts.list", input: {} })).rejects.toBeInstanceOf(RuntimeUnavailableError);
  });
});
...
describe("webhook ingestion — verify, translate, idempotent replay", () => {
  it("ingests a signed webhook then treats a replay as a duplicate", async () => {
    const inst = await connectCrm("hubspot", { webhookSigningSecret: "whsig" });
    const rawBody = JSON.stringify({ id: "evt_1", subscriptionType: "contact.creation" });
...
describe("polling — replay safety", () => {
  it("advances the cursor and treats a repeated start-cursor as a no-op", async () => {
    const inst = await connectCrm("salesforce", { instanceUrl: "https://acme.my.salesforce.com" });
    const first = await pollConnector(ctx, { installationId: inst.id, limit: 2 });
    expect(first.eventCount).toBe(2);
    expect(first.cursor).toBe("2");
    // The installation cursor advanced; polling again resumes from the new cursor.
